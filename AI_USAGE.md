...
16. **Config comes from `.env`** and is exposed as constants by `config/App.php`. Required keys: `ENVIRONMENT`, `AUTH_SALT`, `BUSINESS_NAME`, `REPORT_ERROR_EMAIL`, and `DATABASE_HOST/NAME/USERNAME/PORT/CHARSET`. Auth adds `JWT_SECRET`, `JWT_TTL`, `REFRESH_TTL`. The admin datatable CRUD adds the `DT_*` keys (see that section). `CORS_ALLOWED_ORIGINS` is optional (empty = no cross-origin access to `/api/`; comma-separated origins, or `*`). Missing required keys throw `ConfigurationException` at boot.
